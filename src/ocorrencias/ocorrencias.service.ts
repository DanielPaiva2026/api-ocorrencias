...
export class OcorrenciasService {
  ...
  private async getHorarioEntradaColab(tx: any, colabId: string): Promise<{ hora: number, minuto: number }> {
  ...
  async create(data: any) {
    ...
    if (ocorrencia.colab_id && (ocorrencia.tipo === 'Ausência' || ocorrencia.tipo === 'Falta' || ocorrencia.tipo === 'Atraso')) {
      ...
      // Busca o posto do colaborador para enviar aos supervisores (pega a primeira alocação)
  ...
  async calcularTipoApontamento(colabId: string): Promise<string> {
    const colab = await this.prisma.dBColab.findUnique({
      where: { id: colabId },
      include: { alocacoes: { include: { posto: true } } }
    });
    if (!colab) return 'Extra';

    const tipoContrato = (colab.tipo_contratacao || '').toUpperCase();
    if (tipoContrato.includes('INTERMITENTE')) return 'Trabalho Intermitente';
  ...
  async processWebhook(payload: any) {
    // Exemplo de payload esperado da integração do WhatsApp:
    // { colab_id: "...", tipo: "Falta", observacao: "Enviado pelo WhatsApp" }
    
    // Aqui processamos o payload e mapeamos para o banco
  ...
  async anexarDocumento(id: string, urlDocumento?: string) {
    return this.prisma.fluxoCorretivo.update({
      where: { id },
      data: {
  ...
  async converterParaInjustificada(id: string, sancao: string) {
    return this.prisma.fluxoCorretivo.update({
      where: { id },
      data: {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      if (sancao && sancao !== 'Nenhuma') {
        ...
        if (sancao.includes('Suspensão')) {
          diasAfastamento = 1;
          if (sancao.includes('2 Dias')) diasAfastamento = 2;
          if (sancao.includes('3 Dias')) diasAfastamento = 3;
        ...
        if (diasAfastamento > 0 && ocorrencia.colab_id) {
            ...
            dataRetorno.setUTCHours(horario.hora + 3, horario.minuto, 0, 0); // Fuso horário de Brasília
  ...
  async getSancaoSugerida(colabId: string, tipo: string) {
    let tiposQuery = [tipo];
    ...
    if (count_sancoes === 1) sugerida = 'Advertência';
    if (count_sancoes === 2) sugerida = 'Suspensão 1 Dia';
    if (count_sancoes === 3) sugerida = 'Suspensão 2 Dias';
    if (count_sancoes === 4) sugerida = 'Suspensão 3 Dias';
    if (count_sancoes >= 5) sugerida = 'Justa Causa';
  ...
  async registrarTratamentoAtraso(payload: any) {
    return this.prisma.$transaction(async (tx) => {
      // 1. Grava a ocorrência principal (Atraso ou Falta se não for pegar o posto)
      ...
      // 2. Criação de Afastamento e atualização do status de disponibilidade
      let diasAfastamento = 0;
      let motivoAfastamento = '';

      if (payload.sancao && payload.sancao.includes('Suspensão')) {
        diasAfastamento = 1;
        if (payload.sancao.includes('2 Dias')) diasAfastamento = 2;
        if (payload.sancao.includes('3 Dias')) diasAfastamento = 3;
        motivoAfastamento = 'Suspensão';
      ...
      if (diasAfastamento > 0) {
        ...
        dataRetorno.setUTCHours(horario.hora + 3, horario.minuto, 0, 0); // Fuso horario de Brasilia
      ...
      // 3. Grava as ocorrências Extra (substitutos ou porteiro que aguardou)
      const extras = [];
      if (payload.vai_pegar_posto && payload.gerar_extra && payload.extra_colab_id) {
        // Caso simples: atrasou mas vai pegar o posto. Extra para quem aguardou.
        ...
        // Multiplos substitutos para os dias da falta/suspensão
        ...
        // 3.B. Criar Ocorrência de Treinamento NR se necessário
        if (payload.observacao_substituto && payload.observacao_substituto.includes('[ATENÇÃO: Necessita Treinamento NR]')) {
          const subsUnicos = Array.from(new Set(payload.substitutos.map((s: any) => s.colab_id)));
          for (const subId of subsUnicos) {
            await tx.fluxoCorretivo.create({
              data: {
                colab_id: subId as string,
                tipo: 'Treinamento NR',
                data: new Date(),
  ...
  async registrarTratamentoJornadaIncompleta(payload: any) {
    return this.prisma.$transaction(async (tx) => {
      // 1. Grava a ocorrência principal
      const tipoPrincipal = payload.tipo_jornada; // 'Sair mais cedo' ou 'Chegar mais tarde'
      ...
      // 2. Criação de Afastamento se tiver Suspensão
      let diasAfastamento = 0;
      let motivoAfastamento = '';
      ...
      // 3. Grava a ocorrência Extra se precisar de cobertura e escolheu substituto
      const extras = [];

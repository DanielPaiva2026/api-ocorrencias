...
let FeriasCron = FeriasCron_1 = class FeriasCron {
    ...
    async processarFeriasDoDia() {
        ...
        const avisosIniciandoEm48h = await this.prisma.avisoFerias.findMany({
            where: { data_inicio: { gte: daqui2Dias, lt: daqui3Dias } },
            include: { substituicoes: { where: { ativa: true }, include: { posto: true } } }
        });
        for (const aviso of avisosIniciandoEm48h) {
            for (const sub of aviso.substituicoes) {
                if (sub.posto) {
                    const colabSubstituto = await this.prisma.dBColab.findUnique({ where: { id: sub.colab_substituto_id } });
                    if (colabSubstituto)
        ...
        const avisosTerminandoEm5Dias = await this.prisma.avisoFerias.findMany({
            where: { data_fim: { gte: daqui5Dias, lt: daqui6Dias } },
            include: { colab: true }
        });
        ...
        await this.verificarAlertasDeVencimento(hoje);
        const avisosIniciandoHoje = await this.prisma.avisoFerias.findMany({
            where: { data_inicio: { gte: hoje, lt: amanha } },
            include: { colab: true, substituicoes: { where: { ativa: true } } }
        });
        for (const aviso of avisosIniciandoHoje) {
            await this.prisma.dBColab.update({ where: { id: aviso.colab_id }, data: { situacao_disponibilidade: 'FÉRIAS' } });
            for (const sub of aviso.substituicoes) {
                const alocacaoTitular = await this.prisma.alocacao.findFirst({ where: { posto_id: sub.posto_id, colab_id: aviso.colab_id } });
                if (alocacaoTitular)
                    await this.prisma.alocacao.delete({ where: { id: alocacaoTitular.id } });
                const alocacaoAnteriorSubstituto = await this.prisma.alocacao.findFirst({ where: { colab_id: sub.colab_substituto_id } });
                if (alocacaoAnteriorSubstituto)
                    await this.prisma.alocacao.delete({ where: { id: alocacaoAnteriorSubstituto.id } });
                await this.prisma.alocacao.create({ data: { posto_id: sub.posto_id, colab_id: sub.colab_substituto_id } });
                await this.prisma.dBColab.update({ where: { id: sub.colab_substituto_id }, data: { situacao_disponibilidade: 'Alocado' } });
            }
        }
        const avisosTerminandoHoje = await this.prisma.avisoFerias.findMany({
            where: { data_fim: { gte: hoje, lt: amanha }, status_retorno: 'RETORNA_AO_POSTO' },
            include: { substituicoes: { where: { ativa: true } } }
        });
        for (const aviso of avisosTerminandoHoje) {
            await this.prisma.dBColab.update({ where: { id: aviso.colab_id }, data: { situacao_disponibilidade: 'Alocado' } });
            for (const sub of aviso.substituicoes) {
                const alocacaoSubstituto = await this.prisma.alocacao.findFirst({ where: { posto_id: sub.posto_id, colab_id: sub.colab_substituto_id } });
                if (alocacaoSubstituto) {
                    await this.prisma.alocacao.delete({ where: { id: alocacaoSubstituto.id } });
                    await this.prisma.dBColab.update({ where: { id: sub.colab_substituto_id }, data: { situacao_disponibilidade: 'Livre' } });
                ...
                await this.prisma.alocacao.create({ data: { posto_id: sub.posto_id, colab_id: aviso.colab_id } });
                await this.prisma.substituicaoFerias.update({ where: { id: sub.id }, data: { ativa: false } });
            }
        }
        this.logger.log('Processamento diário de férias concluído.');
    }
    async verificarAlertasDeVencimento(hoje) {
        const colabs = await this.prisma.dBColab.findMany({
            where: {
                OR: [{ status_cadastro: 'Ativo' }, { status_cadastro: null }],
                afastamentos: { none: { motivo: 'INSS', data_inicio: { lte: hoje }, OR: [{ data_fim: null }, { data_fim: { gte: hoje } }] } }
            },
        ...
        for (const c of colabs) {
            ...
            if (!baseDataStr)
                continue;
            const parts = baseDataStr.split('/');
            const dataBase = parts.length === 3 ? new Date(parseInt(parts[2]), parseInt(parts[1]) - 1, parseInt(parts[0])) : new Date(baseDataStr);
            ...
            if (diasRestantesAviso === 60 || diasRestantesAviso === 30 || diasRestantesAviso === 15 || diasRestantesAviso === 0) {
                this.logger.log(`Alerta de férias para ${c.nome}! Faltam ${diasRestantesAviso} dias para limite do aviso.`);
                await this.notificationService.notificarAlertaFerias(c.nome, dataLimiteAviso, diasRestantesAviso);
            }
        }
    }
};
